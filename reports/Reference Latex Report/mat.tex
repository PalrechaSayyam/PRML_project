...
\begin{figure}[!t]
    \centering
    \includegraphics[width=\columnwidth]{images/approach_flowchart_mid.png}
    \caption{Approach: Benchmarking workflow for ML and GNN models.}
...
% \begin{figure*}
%     \centering
%     \includegraphics[width=1.5\columnwidth]{images/hybrid model.png}
%     \caption{Hybrid GNN-RF architecture used for final modeling.}
%     \label{fig:hybrid}
% \end{figure*}
...
%     \centering
%     \includegraphics[width=\linewidth]{images/approach_flowchart_mid.png}
%     \caption{Benchmarking workflow for ML and GNN models.}
%     \label{fig: benchmark_pipeline}
% \end{minipage}
% \hfill
% \begin{minipage}[t]{0.48\textwidth}
%     \centering
%     \includegraphics[width=\linewidth]{images/hybrid model.png}
%     \caption{Hybrid GNN-RF architecture used for final modeling.}
%     \label{fig:hybrid}
% \end{minipage}
% \end{figure*}
...
The Tox21 dataset, part of the Tox21 Data Challenge and later incorporated into MoleculeNet\cite{wu2018moleculenet}, contains \textbf{12 binary toxicity prediction tasks} covering nuclear receptor (NR) signaling pathways and stress response (SR) pathways.  
The dataset includes approximately \textbf{8,000 compounds}, each annotated with SMILES strings and assay-level toxicity labels.  
As is typical in computational toxicology, the dataset exhibits:
\begin{itemize}
    \item \textbf{severe class imbalance} (positive class often $<10\%$),
    \item \textbf{missing labels} for certain assay-compound pairs,
    \item \textbf{heterogeneous difficulty} across toxicity mechanisms.
\end{itemize}
To ensure consistency across all experiments, DeepChem's standardized train/validation/test scaffold split was used.


\subsection{Benchmarking Classical ML Models}
...
Baseline performance using classical machine learning techniques on ECFP fingerprints was first established.  
ECFP4 fingerprints (radius 2, 1024 bits) were generated using DeepChem.  
Three ML models were benchmarked:
\begin{enumerate}
...
\subsection{Model Optimization: SMOTE and Optuna}

To strengthen the classical ML baselines, the following techniques were applied:
\begin{itemize}
    \item \textbf{SMOTE oversampling} to balance the minority class,
    \item \textbf{Optuna hyperparameter tuning} to optimize RF and XGBoost.
\end{itemize}
The following search space was explored via Optuna:
\begin{itemize}
    \item RF: (max\_depth, n\_estimators, min\_samples\_split, min\_samples\_leaf),
    \item XGBoost: (learning\_rate, max\_depth, subsample, colsample\_bytree).
\end{itemize}
...
\begin{figure*}[!t]
    \centering
    \includegraphics[width=\textwidth]{images/hybrid model v3.png}
    \caption{Hybrid GNN-RF architecture used for final modeling.}
    \label{fig:hybrid}
\end{figure*} 

\subsection{Benchmarking GNN Models}
To investigate deep-learned molecular representations, two GNN architectures were evaluated: GraphConv and GAT.

\subsubsection{GraphConv Model}
The Graph Convolutional Network was implemented using DeepChem's \texttt{GraphConvModel}.  
Molecules were represented as molecular graphs with atoms as nodes and bonds as edges.  
The model consisted of two graph convolutional layers with hidden dimensions of 64 and 128 units, followed by a dense layer of 128 units with ReLU activation. 
...
The Graph Attention Network (GAT) was built using DeepChem's \texttt{GATModel}, employing two attention-based graph layers with 64 hidden units and four attention heads per layer to capture 
...
%---------------------------------------------------------
\subsection{Task Hardness: INT-CHEM and EXT-CHEM}
To understand the variability in performance across tasks, task difficulty was quantified following the framework proposed by Fooladi et al.~\cite{doi:10.1021/acs.jcim.4c00160}.

\subsubsection{INT-CHEM: Intrinsic Task Hardness}
INT-CHEM measures the internal difficulty of a task and is defined as:
\[
\text{INT-CHEM} = 1 - \text{PR-AUC}_{\text{few-shot}}
\]
Few-shot single-task models (RF and kNN) were trained on small subsets (512, 1024, 2048 samples) and averaged performance.  
Higher INT-CHEM $\Rightarrow$ harder task.

\subsubsection{EXT-CHEM: External Hardness (Task Relatedness)}
EXT-CHEM measures similarity to other tasks via the \textbf{Optimal Transport Dataset Distance (OTDD)\cite{2025sotdd}}.  
For each pair of tasks, OTDD was computed between:
\begin{itemize}
    \item ChemBERTa molecular embeddings (feature space),
    \item Binary labels (label space).
\end{itemize}

\subsection{Hybrid GNN-RF Model}
...
\subsubsection{Stage 1: Hybrid Feature Generation}
A single, high-dimensional feature vector was created for each molecule by concatenating two feature sets.
\begin{itemize}
    \item \textbf{Learned Graph Embeddings:} A \texttt{GraphConvModel} from the \texttt{DeepChem} library was trained on the complete multi-task Tox21 training set. a validation callback was employed monitoring PR-AUC to save the model's best-performing parameters. This trained GNN was then used as a feature extractor, applying its \texttt{predict\_embedding} method to generate a dense, $N$-dimensional graph embedding for each molecule.
    \item \textbf{Engineered Fingerprints:} A 1024-dimensional Extended-Connectivity Fingerprint (ECFP, radius 2) was computed for each molecule using the \texttt{CircularFingerprint} featurizer.
\end{itemize}
Molecules that failed featurization by either method were removed from all datasets to ensure perfect alignment. The resulting feature sets were horizontally concatenated, forming a single hybrid feature vector of size $N(256)+1024$ for each successfully featurized molecule.
\subsubsection{Stage 2: Task-Specific Random Forest Classifiers}
...
All models were evaluated on the \textbf{validation set} using:
\begin{itemize}
    \item PR-AUC (primary metric),
    \item ROC-AUC,
    % \item Accuracy, Precison, Recall (secondary metrics).
\end{itemize}
For evaluating model performance on the Tox21 dataset, primarily focus was on the PR-AUC. This choice is motivated by the inherent class imbalance prevalent across many Tox21 assays, where the positive class (toxic compounds) is significantly rarer than the negative class (non-toxic compounds). Unlike the ROC-AUC, which can present an overly optimistic view in the presence of severe imbalance, PR-AUC offers a more reliable assessment of a model's ability to correctly identify the minority class while minimizing false positives.
Task-hardness descriptors were analyzed against model performance to interpret why certain tasks favored GNNs, ML models, or hybrid features.  
The hybrid model showed strong improvements particularly for \textbf{high INT-CHEM tasks}, validating the hypothesis that combining global graph features with local fingerprints resolves representational gaps present in GNN-only or fingerprint-only models.
